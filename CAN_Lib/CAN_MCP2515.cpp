...
void CAN_MCP2515::TEST(uint8_t c) {
  CAN_Frame tmpFrame;
  tmpFrame.cmd = TEST_DATA;
  tmpFrame.hash = UID_BASE;
  tmpFrame.resp_bit = true;
  tmpFrame.length = 3;
  tmpFrame.data[0] = c;
  tmpFrame.data[1] = TEST_DATA;
  tmpFrame.data[2] = TEST_DATA;
  sendCanFrame(tmpFrame);
}

//Initialize SPI communications and set MCP2515 into Config mode
CAN_MCP2515plus::CAN_MCP2515plus()
{
  //Use a default of pin 10 for SPI chip select
  CS = 10;
  _init();
...
CAN_MCP2515plus::CAN_MCP2515plus(uint8_t CS_Pin)
{
  CS = CS_Pin;
  _init();
}
void CAN_MCP2515plus::can_answer2(uint8_t lng, bool resp){
...
void CAN_MCP2515plus::can_answer(uint8_t lng){
  can_answer2(lng, true);
}

void CAN_MCP2515plus::configTerminator(int channel, int framecount) {
  outgoingMsg.cmd = CONFIG_Status;
  for (uint8_t i = 0; i < 4; i++) {
    outgoingMsg.data[i] = params.uid_device[i];
  }
  outgoingMsg.data[4] = channel;
  outgoingMsg.data[5] = framecount;

  can_answer(6);
}
void CAN_MCP2515plus::configDataFrame(uint8_t config_data[8], int framecount) {
  outgoingMsg.cmd = CONFIG_Status;
  outgoingMsg.hash = framecount + 1;
  outgoingMsg.resp_bit = true;
  outgoingMsg.length = 8;
  for (int i = 0; i < 8; i++) {
    outgoingMsg.data[i] = config_data[i];
  }
  sendCanFrame(outgoingMsg);
  delay(1);
}

CAN_MCP2515 CANBase(10); // Create CAN channel using pin 10 for SPI chip select
CAN_MCP2515plus CAN(10); // Create CAN channel using pin 10 for SPI chip select
