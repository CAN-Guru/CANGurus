...
#define rightpos  1
#define endpos  4
#define maxservodelay 50
#define stdservodelay maxservodelay/2
...
public:
  ...
  void Init(int pin, int d, position togo);
  void Detach();
  void Update();
  void GoLeft();
  void GoRight();
  uint16_t GetLocID();
  void SetLocID(uint16_t locID);
  uint8_t GetRegID();
  void SetRegID(uint8_t regID);
  position GetPosDest();
